...
def process_video_url(url: str) -> str:
    """
    Process different URL formats to make them suitable for direct file sending
    """
    if not url:
        return url
    ...
    if "pixeldrain.com" in url or "pixeldrain.dev" in url:
        ...
        if "/api/file/" in url:
            ...
            # Convert from share format to API format
    ...
    # Handle MediaFire URLs
        ...
        # MediaFire requires special handling, but we'll try direct approach
        pass
    
    # Handle Mega URLs
    elif "mega.nz" in url:
        # Mega URLs usually work directly
        pass
    
    # Handle WeTransfer URLs
    elif "wetransfer.com" in url:
        # WeTransfer URLs work directly
        pass
    
    # Add more URL processors as needed
...
async def get_file_size(url: str) -> tuple:
    ...
    Returns: (size_in_bytes, content_type, is_streamable)
    """
    try:
...
async def send_video_file(client: Client, chat_id: int, video_url: str, 
    ...
    """
    Enhanced function to send video files with multiple fallback methods
    Returns True if successful, False otherwise
    """
    try:
        # Process URL for direct access
        processed_url = process_video_url(video_url)
        logger.info(f"Processed URL: {processed_url}")
        
        # Get file information
        file_size, content_type, is_video = await get_file_size(processed_url)
        ...
        caption += (
            ...
            "**⚠️ This file will be auto-deleted in 10 minutes**\n\n"
            "**Created By:** [Zero Creations](https://t.me/zerocreations)"
        )
        
        # Prepare reply markup
        buttons = InlineKeyboardMarkup([
            [InlineKeyboardButton("🚀 DIRECT DOWNLOAD", url=processed_url)],
            [InlineKeyboardButton("⭐ RATE", callback_data=f"rate_{media['id']}_{quality or 'auto'}")]
        ])
        
        # Method 1: Try sending as video (best for small files)
        if file_size < 50 * 1024 * 1024 and is_video:  # Less than 50MB
            try:
        ...
        # Method 2: Try sending as document (more reliable for larger files)
        try:
            ...
            logger.warning(f"Failed to send as document: {e}")
        
        # Method 3: Try with custom filename
        try:
            logger.info("Attempting to send with custom filename...")
            filename = f"{media.get('title', 'movie')}.mp4"
            sent_msg = await client.send_document(
                chat_id=chat_id,
                document=processed_url,
                file_name=filename,
            ...
            logger.info("Successfully sent with custom filename")
            return sent_msg
        except Exception as e:
            logger.warning(f"Failed to send with custom filename: {e}")
        
        # Method 4: Send as animation (for certain formats)
        try:
            logger.info("Attempting to send as animation...")
            sent_msg = await client.send_animation(
                chat_id=chat_id,
                animation=processed_url,
                caption=caption,
                reply_markup=buttons
            )
            logger.info("Successfully sent as animation")
            return sent_msg
        except Exception as e:
            logger.warning(f"Failed to send as animation: {e}")
            
    except Exception as e:
        logger.error(f"All sending methods failed: {e}")
...
async def handle_callback_query(client: Client, callback_query: CallbackQuery):
    ...
    if data.startswith("result_page_"):
        ...
        if sent_msg:
            ...
            await callback_query.edit_message_text(
                f"✅ **{media.get('title', 'N/A')} - {quality.upper()}** has been sent to your chat!\n\n"
                "**⚠️ The file will be auto-deleted in 10 minutes. Save it quickly!**",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 Back to Search", callback_data="back_to_search")]
                ])
            )
        else:
            # Fallback to download link if direct sending fails
            message_text = (
        ...
        if sent_msg:
            ...
            # Update the callback message
            await callback_query.edit_message_text(
                f"✅ **{media.get('title', 'N/A')} S{season_num}E{episode_num}** has been sent to your chat!\n\n"
                "**⚠️ The file will be auto-deleted in 10 minutes. Save it quickly!**",
                reply_markup=InlineKeyboardMarkup([
        ...
        if data == "top_searches":
            ...
            text = (
                ...
                "• All files auto-delete in 10 minutes\n"
